...
flowchart TD
    ...
    Orchestrator --> R1["第一轮：创作起点 + 核心体验<br/>为什么想写 / 读者读完后的感觉（合并）"]
    R1 --> R2["第二轮：主角灵魂<br/>想要什么 / 怕什么 / 矛盾"]
    R2 --> R3["第三轮：品类基调 + 篇幅模式<br/>给建议 + 用户确认（合并）"]

    R3 --> Confirm{"复述确认<br/>用户认可？"}
    Confirm -->|"✗ 调整"| R1
...
```
第一轮 — 创作起点 + 核心体验（合并，O12 修复——减少冗余对话）：
  先聊聊你想写的故事吧。可以不完整、可以只是一些碎片——
  比如某个场景、某个人物、某种感觉。或者对市面上某类书不满意想自己写。
  
  同时：你希望读者读完这本书后，心里留下什么感觉？
  （爽、燃、虐、悬疑感、被治愈、想了很多天）
...
第三轮 — 品类基调 + 篇幅模式（合并，O12 修复）：
  从你说的来看，天然适合 [品类]——但基调我想确认：偏 [A/B/C] 还是混合？
  篇幅推荐 [建议]，模式推荐 [建议]。你觉得呢？
