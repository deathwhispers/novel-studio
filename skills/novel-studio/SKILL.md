...
11. 用户已经有设定和章节计划，想写下一章、下一场或续写正文
   - 路由到 `novel-chapter-workflow`（强制执行完整的章节写作流程）
...
14. 用户只需要简单的草稿起草，不需要完整流程
   - 路由到 `novel-drafting`（仅当用户明确要求跳过完整流程时）
...
- **章节写作强制流程**：当用户请求写新章节、续写正文、写下一章时，优先路由到 `novel-chapter-workflow`，它会强制执行完整的质量门禁流程（草稿→检查→修正→去AI味→状态回写）。
- **简单草稿请求**：只有当用户明确要求跳过完整流程、只需要简单起草时，才路由到 `novel-drafting`。
