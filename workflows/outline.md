...
flowchart TD
    ...
    Layer2["第二层：逐卷节拍（可选）<br/>卷内位置→线间交错→节奏检查"]
    Layer2 -.->|"可跳过"| Layer3
    Layer3["第三层：伏笔布局（可选）<br/>埋设卷→轻碰→回收卷"]
    Layer3 -.->|"可跳过"| Layer4
    Layer4["第四层：角色弧光（可选）<br/>卷状态变化→催化剂位置"]
    Layer4 -.->|"写作中按需补"| Write
...
第一层确认后，Orchestrator 将确认的内容传递给 Outliner，生成全书总纲 + 分卷文件。此后即可 `/novel-studio:write` 开写，或继续向下细化（第二/三/四层均为可选）。

### 第二层：逐卷细化（每卷 2-3 轮对话，可选）
...
- **总纲即可开写**：第一层（全书总纲 + 分卷）确认后即可 `/novel-studio:write`，第二/三/四层（逐卷节拍/伏笔/弧光）在写作过程中按需渐进补，不强制一步到位
- **规划到卷，不规划到章**：伏笔/节拍/弧光/交错只标注到「第几卷 + 卷内前/中/后段」，不预先钉死章号
