...
description: "旧版工作区升级流程。将 v2 旧结构（作品总表 + 硬设定 + hard_canon）无损升级到 v3（作品核心 + 硬规则 + hard_rules + schema_version），保留 state/ 续写上下文。"
---

# upgrade-project — 工作区升级 Workflow

> 与 migrate-project 的区别：migrate 针对「裸正文目录」（无 core/state，需重建）；upgrade 针对「旧版已初始化工作区」（有旧结构 + state/，只改结构不改状态，绝不重扫正文）。
...
flowchart TD
    ...
    Orchestrator --> Step1["💾 步骤1：备份<br/>progress.yaml + core/ + setting/<br/>→ _upgrade-backup/"]
    Step1 --> Step2["🔧 步骤2：结构改名<br/>作品总表→作品核心<br/>硬设定→硬规则<br/>progress.yaml 字段改写"]
    Step2 --> Step3["💬 步骤3：内容补全<br/>Architect + 作者确认<br/>补灵魂契约 / 拆硬设定"]
...
- Orchestrator 检测到：`core/作品总表.md`（旧名）存在，且 `state/progress.yaml` 缺失 `schema_version` 字段（或 `files` 块使用旧字段 `hard_canon`）
...
**不动的部分**：`workspace.name/genre/mode/created`、`current.*`、`chapter_state.*`、`next_milestone.*`、`files.outline/volumes/chapters_dir` 全部原样保留。

---

## 步骤 3：内容补全（Architect + 作者确认）
...
3. progress.yaml 含 schema_version: 3、files.hard_rules、workspace.chapter_word_target
4. state/ 下 5 个大文件原样存在，未被改动
5. 无残留旧路径（grep 无 作品总表 / 硬设定 / hard_canon）
```
...
| 检测到 v1（中文数字目录） | 引导走 migrate（v1 结构差异过大，按裸目录重建） |
