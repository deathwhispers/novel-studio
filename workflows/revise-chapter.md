...
| 用户描述 | 修订范围 | 跳过 | 说明 |
|---------|---------|------|------|
| 「重写第X章」「全部重写」 | 全文重写 | 无 | 等同于 write-chapter |
| 「第X章节奏不对」「场景结构有问题」 | 场景重设 | Director | 重排场景结构，保持核心功能不变 |
| 「有几处写得不好」「对话修一下」 | 局部修复 | Director, ScenePlanner | 只改标记位置，不动其他 |
| 「AI味太重」「去味」 | 仅去味 | Director, ScenePlanner | 只修AI味关键词，不动剧情结构 |
...
2. **修订不能改变章节的硬设定**（不能突然改了力量等级或角色关系，也不能改变系统面板的字段/标题/格式——面板以 `setting/系统面板.md` 为准）
