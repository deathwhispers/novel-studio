...
ORCHESTRATOR 动作:
  ...
  3. 如果是新章节:
     ...
     e. 在生成选项之前，先问用户：这一章有没有特别想写的东西？还是交给 Orchestrator 来想？
        - 如果用户有具体想法 → 基于用户想法生成选项
        - 如果用户交给你 → 基于当前叙事状态 + 品类节奏要求，生成 2-3 个选项
     f. 展示选项（每个选项保持简洁：方向名/一句话核心/2-3个关键事件/章尾感觉/一个优势一个风险）
     g. 等待用户选择（用户可单选、混合、或提出自己的方案）
     h. 剧情量评估:
        - 如果用户方向预估字数 > 3000 字或关键事件 > 5 个
        - → 建议拆分为连续两章，标注分章点和每章字数预估
        - → 用户确认后，当前章节只写前半部分，剩余排入下一章队列
     i. 用户确认 → 写入 progress.yaml: chapter_state = NEED_PLAN
