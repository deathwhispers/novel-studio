...
ORCHESTRATOR 动作:
  ...
  3. 如果是新章节:
     ...
     d. 根据当前叙事状态 + 品类节奏要求，生成 2-3 个发展方向选项:
        每个选项包含：
        - 方向: 本章核心功能（推进 / 揭示 / 高潮 / 过渡 / 余震）
        - 一句话梗概: 这一章讲什么
        - 关键事件: 2-3 个主要情节节点  
        - 预估字数: 基于事件数量估算（每个事件 ≈ 500-800 字）
        - 章尾落点: 这一章结束时读者最想知道什么
        - 优劣: 这个方向的优势与风险
     e. 展示选项，等待用户选择（用户可单选、混合、或提出自己的方向）
     f. 剧情量评估:
        - 如果用户方向预估字数 > 3000 字或关键事件 > 5 个
        - → 建议拆分为连续两章，标注分章点和每章字数预估
        - → 用户确认后，当前章节只写前半部分，剩余自动排入下一章队列
     g. 如果品类配方可用：标注品类节奏要求
     h. 用户确认 → 写入 progress.yaml: chapter_state = NEED_PLAN
