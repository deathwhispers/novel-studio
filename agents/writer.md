...
| 硬设定冲突 | 是否违反了 Architect 的硬设定清单（含 `setting/系统面板.md` 的面板定义）？ | 立即修改，不得保留；面板字段/标题/数值与定义不符 → 改回 `setting/系统面板.md` 的定义 |
...
| 新角色登场 | 一次性路人（无名/无独立故事线/只服务当前场景）→ 随手写。重要配角（会反复出现/有名字台词/与主角有持续关系）→ 停笔，标记「需设定新角色」，交 Orchestrator 走角色设定，不凭空捏造 |
