...
| 按需加载 | 单个 narrative skill（每次只加载 1 个）、品类 tropes、`references/double-entendre-catalog.md`（都市/暧昧场景）、`references/web-novel-formatting.md`（排版硬约束，启动时必加载）、`setting/系统面板.md`（系统爽文品类，写系统界面时必加载） |
| 绝不加载 | 完整 Scene Contract、完整大纲、完整 canon、状态文件、chapter N-2 全文 |
| 决策权 | 句子级写作、叙述节奏、对话设计、Skill 调用时机 |
| 禁止行为 | 触碰禁止清单、读大纲、自行决定信息释放、修改 canon、强行制造钩子（章尾反转/悬念需从情节中自然生长，不为断章而断章）、在事件高潮处强行断章（按字数自然收束，找情节的自然停顿点结束，禁止没头没尾的割裂语句）、假装洞察（用「不是A。是B。」句式假装有观察力，直接写肯定句）、假装极简（用独立短句「X到了」做时间跳跃但无感官锚点，极简需要句子有重量）、故作留白（对白中用省略号「……」代替实际回应，写清楚回避动作而非扔一串省略号）、替读者判断（写一个薄动作加「不像X——像在Y」来解释含义，删掉解释句，把感受写进动作本身） |
...
- 「不是……。是……」→ 是否是角色真正的自我纠错？不是 → 删否定直写肯定
- 「不像……——像……」「像是在……」→ 动作本身是否已经让读者能感受到？是 → 删掉解释句；否 → 把动作写厚
- 「……」（对白中的省略号）→ 是否在回避写实际内容？是 → 写清楚回避动作
- 独立短句「X到了」「天亮了」→ 有画面和感官锚点吗？没有 → 加或删

**阈值**：3 处以上 → 逐项修复后重读；2 处同类 → 重点训练；1-2 处 → 修复即可通过。

### 第六步：活人感速检

AI 味检测只能告诉你「删了什么」——活人感速检告诉你「缺了什么」。写完一段即自查，写完一章再通查一次。

**5 个自检问题**：
1. **有闲笔吗？** 角色有没有做与剧情无关的动作（喝水、看窗外、挠头、哼歌）？没有 → 加一个
2. **用了几种感官？** 连续 300 字只有视觉描写？→ 注入一个触觉（温度/材质）或听觉（环境音/人声）
3. **角色心里有声音吗？** 角色有没有吐槽/骂人/自言自语/走神/说半句话？没有 → 考虑加一句
4. **叙事者像监控摄像头吗？** 客观得像新闻稿？→ 换成角色自己的眼睛看世界：有偏见、有盲点、有私人判断
5. **有没有机械句式？** 搜索「第一/第二/首先/其次/不是…是…/不像…——像…/像是在…/没有…/……（省略号对白）/X到了（独立时间跳跃）」→ 有则参照 `references/de-flavor-techniques.md` 修复

**快速诊断**：随机抽 3 段读一遍——
- 如果感觉像在看监控录像（画面清晰但毫无温度）→ 缺闲笔 + 缺感官轮换
- 如果感觉像在读范文（工整正确但不吸引人）→ 缺内心声音 + 叙事者太客观
- 如果感觉像在读说明文（结构清晰但没有情绪）→ 缺身体反应 + 缺偏见
