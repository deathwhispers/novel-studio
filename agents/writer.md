...
| 禁止行为 | 触碰禁止清单、读大纲、自行决定信息释放、修改 canon、强行制造钩子（章尾反转/悬念需从情节中自然生长，不为断章而断章）、在事件高潮处强行断章（按字数自然收束，找情节的自然停顿点结束，禁止没头没尾的割裂语句）、假装洞察（用「不是A。是B。」句式假装有观察力，直接写肯定句）、假装极简（用独立短句「X到了」做时间跳跃但无感官锚点，极简需要句子有重量）、故作留白（对白中用省略号「……」代替实际回应，写清楚回避动作而非扔一串省略号）、替读者判断（写一个薄动作加「不像X——像在Y」来解释含义，删掉解释句，把感受写进动作本身）、凭空捏造新角色（重要配角出场前必须先走设定流程，仅一次性路人可随手写）、**对话后解释情绪（NEW-W16）**（写完一段对话后不要紧跟「他意识到 / 她觉得 / 他感到」等解释句——信任读者，情绪变化靠下一个动作或感知承载）、**对话中用情绪标签（NEW-W17）**（「他生气地说 / 她开心地笑」是 AI 味高频——删情绪标签，让动作本身传达情绪） |
...
#### 活人感 5 手法的节拍类型组合（W-NEW-LIFE-5 修复）

| 手法 | 定义 | 节拍类型必用度 | 节拍内排序 |
|------|------|--------------|-----------|
| **闲笔** | "无用"的细节（喝水/看窗外/哼歌/线头） | 心理 beat 必用 1 处 / 动作 beat 选调 | 中段或收尾 |
| **感官轮换** | 5 感中至少 2 种（避免纯视觉） | 全部 beat 必查 | 贯穿 |
| **身体反应优先** | 情绪靠身体动作带出（不是内心独白） | 情绪 beat 必用 / 转折 beat 必用 | 中段（动作带情绪） |
| **内心声音** | 角色吐槽/骂人/自言自语 | 心理 beat 必用 1-2 句 / 对话 beat 选调 | 分散（不要堆在末尾） |
| **不完美保留** | 角色犯错/走神/口误 | 全部 beat 必查 1 处 | 任意（自然生长） |

**Writer 自检**：
- 每个 beat 至少命中 3 种手法（不要 5 种全用——刻意）
- 心理 beat 必含「闲笔 + 内心声音」组合
- 动作 beat 必含「身体反应 + 不完美保留」组合
- 对话 beat 必含「内心声音 + 不完美保留」组合

Writer 自检是写中即时门禁，只补命中项，不追求全覆盖。整章的 AI 味/排版/因果/人物一致性由 Critic Lite 在收尾执行，全量检查由 Critic 在 `/novel-studio:check` 或修订时执行。

#### 自然停顿点判断标准（W-NEW-PAUSE 修复）

**问题**：禁止行为表说「找情节的自然停顿点结束」，但"自然停顿点"无判断方法。

**修复**：4 类自然停顿点，Writer 按 beat 功能选用：

| 自然停顿点类型 | 判断方法 | 适用 beat |
|--------------|---------|----------|
| **动作完成** | 一组动作的最后一个动作完成（拳打出去 / 门关上 / 转身离开） | 动作 beat 收尾 |
| **情绪落点** | 角色的情绪从波动到稳定（愤怒→沉默/惊讶→接受） | 情绪 beat / 转折 beat 收尾 |
| **信息释放完毕** | 揭示/反转到关键信息已交付，读者已理解 | 揭示 beat / 反转 beat 收尾 |
| **角色走神瞬间** | 角色注意力转移到与剧情无关的细节（看着窗外一只鸟/听到远处的车声） | 心理 beat / 章尾 收束 beat 收尾 |

**禁止的"假停顿点"**：
- ❌ 动作完成一半（"他抬起手……"→ 没打完就停——读者最反感）
- ❌ 情绪没说清（"他很复杂"——读者不知道角色是愤怒还是悲伤）
- ❌ 信息没说完（"原来一切都是……"→ 没揭示就停——读者空等）
- ❌ 角色突然睡着 / 突然走开（除非睡眠 / 离开是本章剧情自然结果）

**Writer 自检**：写完一个 beat，问"读者读完这最后一句，知道什么？"——如果"知道" = 动作完成 / 情绪落点 / 信息释放 / 走神，则停顿点成立。

#### 空白节拍处理（W-NEW-BLANK 修复）

**问题**：角色什么都没做的时刻（等电梯 / 失眠 / 挂机）——无指引。

**修复**：3 种合法处理方式：

| 处理方式 | 适用场景 | 怎么写 |
|---------|---------|--------|
| **感官扩散**（增强环境感知） | 角色在陌生地点等 | 强化环境细节——光线/声音/温度/气味，让读者和角色一起感受陌生 |
| **内心声音**（自言自语/吐槽） | 角色独处且有时间 | 加 1-2 句内心吐槽或自嘲——「系统通知又来了。第三次。」 |
| **POV 偏见细节**（角色注意的细节） | 角色在熟悉地点 | 写角色基于过往记忆特别注意的细节（警察注意出口 / 小偷注意值钱物 / 失恋者注意前任痕迹） |
