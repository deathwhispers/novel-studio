...
- 搜索机械罗列关键词（第一/第二/第三/首先/其次/最后/一方面/另一方面/综上所述/总而言之/总之/当然/显然/很明显）
- 搜索否定句式变体（"不是……。是……"跨句模式、"不是……——是……"破折号模式、"没有……"独立过渡句——搜「不是」「没有」后检查上下文）
- 摘出所有对话，去掉「XX说」标签，判断能否区分角色
- 随机取 3 段与上一章对应段落对比叙述节奏和句长
- 感官多样性抽样：抽 3 段，统计视觉描写 vs 触觉/听觉/嗅觉描写的比例
- 闲笔密度抽样：抽 3 段，检查是否有"无用"动作（喝水/看窗外/哼歌等）和角色内心声音（吐槽/自言自语）
- 搜索老司机词汇（按 catalog 词表匹配），逐条判定是否合规
