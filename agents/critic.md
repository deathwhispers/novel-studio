...
- [ ] 网络热梗检测（按 `references/lib/internet-meme-catalog.md`）：密度是否超标、人设/品类是否匹配、是否攻击性/侵权梗、是否为刻意堆梗
...
- 网络热梗人设不符/品类禁区（古风仙侠原生角色、正剧沉浸章） → 硬伤，`局部修复`
- 网络热梗密度过高或堆梗（一章 > 2 处） → 软问题，标注修复建议
