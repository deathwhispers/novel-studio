...
另内置 **迪化流** 品类（`genres/迪化流/`）：配角疯狂脑补 + 自我攻略，把主角的反常行为解读成"隐藏很深的豪门大佬微服做事"；主角澄清被越描越黑，换招把结果推向原意反方向，阴差阳错兑现。

- `recipe.md` — 品类核心配方：矛盾机制（澄清必失败）、误会势能体系（L1-L5）、双轴推进（系统进度 + 势能升档）、禁忌清单
- `tropes.md` — 迪化常见变体：次数达成（长期任务）/ 里程碑递进 / 封闭场景 / 真相危机（事故局）
- `rhythm.md` — 番茄快节奏单章法：误会起势 → 澄清失败 → 换招反方向 → 章尾钩子
- `panels.md` — 轻量系统面板（主线进度 + 里程碑 + 调味任务，仅长期目标式系统需要）
- `examples.md` — 脑补证据链 / 澄清失败 / 歪打正着兑现的谱分解
